...
## Version Description: In this version (v2) the neural network outputs 2 correction factors.
...
class PINN(object):
    ...
    def train(self, X, dia, row_id, epsi, MTR_DuctArea, target_df, zeta_col_names,V_max_org):
        ...
        function outputs two correction factors.
            ...
            epsi:          Tupple containing Modified Correction factors for MTR and Duct part (TZ6_zmix)
        ...
        print('\nNN output (C_f):',c_f)
        MTR_cf_pred = c_f[0][0]
        TZ6_zmix_cf_pred = c_f[0][1]

        if (MTR_cf_pred >= 0) and (TZ6_zmix_cf_pred >= 0):
            print('MTR-Cf:',MTR_cf_pred,', TZ6-zmix-Cf:',TZ6_zmix_cf_pred)
            MTR_cf_epsi = MTR_cf_pred + MTR_epsi
            TZ6_zmix_cf_epsi = TZ6_zmix_cf_pred + TZ6_zmix_epsi
        elif (MTR_cf_pred < 0) or (TZ6_zmix_cf_pred < 0):
            print('MTR-Cf:',MTR_cf_pred,', TZ6-zmix-Cf:',TZ6_zmix_cf_pred)
            c_f = abs(c_f)
            print('\nNN output is -ve. Taking |(c_f)| values for both MTR and TZ6_zmix:',c_f)
            if (MTR_cf_pred < 0):
                MTR_cf_epsi = MTR_cf_pred - MTR_epsi
        ...
        _,target_df[MTR+'_Zeta3D'].loc[row_id] = zip(*[SHR_Zeta_3D(1,dia,MTR_DuctArea,MTR_cf_pred)])
        target_df['TZ6_zmix_Zeta3D'].loc[row_id] = float(zeta_values[0]) * TZ6_zmix_cf_pred
        print('New_Zeta for MTR-600:' , target_df[MTR+'_Zeta3D'].loc[row_id].values)
        print('New_Zeta for TZ6_zmix:', target_df['TZ6_zmix_Zeta3D'].loc[row_id].values)
        ...
        ## Pass Neural network output (correction factor) to compute new Zeta and then re-run FDDN solver
        _,target_df[MTR+'_Zeta3D'].loc[row_id] = zip(*[SHR_Zeta_3D(1,dia, MTR_DuctArea, MTR_cf_epsi)])
        target_df['TZ6_zmix_Zeta3D'].loc[row_id] = float(zeta_values[0]) * TZ6_zmix_cf_epsi
        print('New_Zeta_epsi for MTR-600:' , target_df[MTR+'_Zeta3D'].loc[row_id].values)
        print('New_Zeta_epsi for TZ6_zmix:', target_df['TZ6_zmix_Zeta3D'].loc[row_id].values)
        ...
        flowrate_diff = X[:,-1] - V_hat_FDDN_scaled # Difference between the FDDN flowrate and LTR Flow Rate --> Passing X[:,4] may be wrong
        ...
        return HoV,V_hat_FDDN,V_true_LTR,break_flag,c_f[0][0],c_f[0][1]
